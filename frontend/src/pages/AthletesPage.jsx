...
import { Search, Filter, Plus, MoreVertical, Trash2 } from 'lucide-react';
...
export default function AthletesPage() {
  ...
  const filteredAthletes = useMemo(() => {
    return athletes.filter((athlete) => {
      const text = `${athlete.name} ${athlete.sport} ${athlete.position}`.toLowerCase();
  ...
  const handleAddAthlete = (e) => {
    ...
    const created = {
      ...
      risk: newAthlete.risk,
      lastAnalysis: new Date().toISOString().slice(0, 10),
    };

    setAthletes((prev) => [created, ...prev]);
    setNewAthlete({ name: '', sport: 'Basketball', position: 'Player', risk: 'Low' });
    setShowForm(false);
    setMenuOpenId(null);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <Button className="shrink-0" onClick={() => setShowForm((prev) => !prev)}>
      ...
      {showForm && (
        <Card glass className="p-5">
          <form onSubmit={handleAddAthlete} className="grid grid-cols-1 md:grid-cols-4 gap-3">
            <input
              type="text"
              value={newAthlete.name}
              onChange={(e) => setNewAthlete((prev) => ({ ...prev, name: e.target.value }))}
              placeholder="Athlete name"
              className="w-full p-2 border rounded-lg bg-white dark:bg-dark-bg text-gray-900 dark:text-white"
              required
            />
            <input
              type="text"
              value={newAthlete.sport}
              onChange={(e) => setNewAthlete((prev) => ({ ...prev, sport: e.target.value }))}
              placeholder="Sport"
              className="w-full p-2 border rounded-lg bg-white dark:bg-dark-bg text-gray-900 dark:text-white"
            />
            <input
              ...
              onChange={(e) => setNewAthlete((prev) => ({ ...prev, position: e.target.value }))}
              placeholder="Position"
              className="w-full p-2 border rounded-lg bg-white dark:bg-dark-bg text-gray-900 dark:text-white"
            />
            <select
              value={newAthlete.risk}
              onChange={(e) => setNewAthlete((prev) => ({ ...prev, risk: e.target.value }))}
              className="w-full p-2 border rounded-lg bg-white dark:bg-dark-bg text-gray-900 dark:text-white"
            >
      ...
      <Card glass className="overflow-hidden">
              ...
              {filteredAthletes.map((athlete) => (
                <tr key={athlete.id} className="hover:bg-gray-50/50 dark:hover:bg-dark-bg/50 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-right relative">
                    <button
                      type="button"
                      onClick={() => setMenuOpenId((prev) => prev === athlete.id ? null : athlete.id)}
                      className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 p-2 rounded-lg transition-colors"
                    >
                      <MoreVertical className="w-5 h-5" />
                    </button>

                    {menuOpenId === athlete.id && (
                      <div className="absolute right-0 top-full mt-2 z-10 w-40 rounded-lg border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-slate-900">
                        <button
                          type="button"
                          onClick={() => handleRemoveAthlete(athlete.id)}
                          className="w-full flex items-center gap-2 px-3 py-2 text-left text-sm text-red-500 hover:bg-red-500/10"
                        >
                          <Trash2 className="w-4 h-4" /> Remove Athlete
                        </button>
                      </div>
                    )}
                  </td>
        ...
        <div className="p-4 border-t border-gray-200 dark:border-gray-800 flex items-center justify-between bg-gray-50/50 dark:bg-dark-bg/50 text-sm text-gray-500 dark:text-gray-400">
          <span>Showing {filteredAthletes.length} entries</span>
