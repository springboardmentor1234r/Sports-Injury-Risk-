...
const AnalysisHistory = () => {
  ...
  return <div className="space-y-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="text-3xl font-bold text-slate-800 sm:text-4xl">Analysis History</h1><p className="mt-2 text-sm text-slate-600">Search, review, and manage permanently saved analysis reports.</p></div><span className="rounded-full bg-brand-50 px-3 py-1.5 text-sm font-medium text-brand-700">{pagination.total ?? 0} saved analyses</span></div><HistoryFilters filters={filters} athletes={athletes} onChange={setFilters} />{error && <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-500">{error}</div>}{loading ? <div className="rounded-2xl border border-brand-200 bg-white p-8 text-center text-sm text-slate-500 shadow-sm">Loading analysis history…</div> : <AnalysisHistoryTable items={history.items} onView={(id) => navigate(`/analysis-history/${id}`)} onDelete={setDeleteTarget} deletingId={deletingId} />}<div className="flex items-center justify-between"><button disabled={pagination.page <= 1} onClick={() => setFilters({ ...filters, page: filters.page - 1 })} className="rounded-xl border border-brand-300 bg-white px-4 py-2 text-sm font-semibold text-brand-600 shadow-sm transition hover:bg-brand-50 disabled:opacity-40">Previous</button><span className="text-sm font-medium text-slate-600">Page {pagination.page || 1} of {pagination.totalPages || 1}</span><button disabled={!pagination.totalPages || pagination.page >= pagination.totalPages} onClick={() => setFilters({ ...filters, page: filters.page + 1 })} className="rounded-xl border border-brand-300 bg-white px-4 py-2 text-sm font-semibold text-brand-600 shadow-sm transition hover:bg-brand-50 disabled:opacity-40">Next</button></div><DeleteAnalysisModal analysis={deleteTarget} onCancel={() => setDeleteTarget(null)} onConfirm={confirmDelete} loading={Boolean(deletingId)} /></div>;
};
