...
class AthleteProfile(Base):
    ...
    risk_assessments = relationship("RiskAssessment", back_populates="athlete_profile", cascade="all, delete-orphan")
...
class VideoFrame(Base):
    ...
    left_knee_deviation_angle = Column(Float, nullable=True)   # FPPA-style valgus proxy, see biomechanics.py
    right_knee_deviation_angle = Column(Float, nullable=True)
...
class RiskAssessment(Base):
    """
    One computed injury-risk snapshot for an athlete, combining the BRD's
    5 weighted categories (Biomechanical Deviations 35%, Movement Asymmetry
    20%, Historical Injury Factors 20%, Training Load 15%, Fatigue 10%)
    into a single explainable score.

    Important, and worth restating here (not just in the service code):
    this is a HEURISTIC score built on published, cited thresholds (ACWR,
    Limb Symmetry Index, FPPA normative values) -- NOT a trained machine
    learning model, because no labeled injury-outcome dataset exists in
    this system to train one on. It is a screening aid, not a diagnosis,
    and every score is stored with its full breakdown and any data-quality
    warnings so the reasoning is inspectable, not a black box. See
    app/services/risk_scoring.py for the full citation list.
    """
    __tablename__ = "risk_assessments"

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    athlete_profile_id = Column(UUID(as_uuid=True), ForeignKey("athlete_profiles.id"), nullable=False)
    based_on_video_id = Column(UUID(as_uuid=True), ForeignKey("videos.id"), nullable=True)

    overall_score = Column(Float, nullable=False)  # 0-100
    risk_band = Column(Enum(RiskBand), nullable=False)

    biomechanical_score = Column(Float, nullable=True)      # 0-100, contributes 35%
    asymmetry_score = Column(Float, nullable=True)          # 0-100, contributes 20%
    historical_injury_score = Column(Float, nullable=True)  # 0-100, contributes 20%
    training_load_score = Column(Float, nullable=True)      # 0-100, contributes 15%
    fatigue_score = Column(Float, nullable=True)             # 0-100, contributes 10%

    breakdown = Column(JSON, nullable=True)                  # list of human-readable factor explanations
    data_completeness_warnings = Column(JSON, nullable=True)  # list of strings: missing data that reduced confidence

    computed_by_user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
    computed_at = Column(DateTime, default=datetime.utcnow)

    athlete_profile = relationship("AthleteProfile", back_populates="risk_assessments")

    __table_args__ = (
        Index("ix_risk_assessments_athlete_profile_id", "athlete_profile_id"),
    )
