...
let riskAssessmentOverride = null;
let teamOverviewOverride = null;
  ...
  return {
    default: {
      get: vi.fn((url) => {
        ...
        if (url.includes("/assessments") && !url.includes("risk-assessments")) return Promise.resolve({ data: [] });
        if (url.includes("/training-load")) return Promise.resolve({ data: [] });
        if (url.includes("/risk-assessments/latest")) {
          return riskAssessmentOverride
            ? Promise.resolve({ data: riskAssessmentOverride })
            : Promise.reject({ response: { status: 404 } });
        }
        if (url.includes("/risk-assessments")) {
          return Promise.resolve({ data: riskAssessmentOverride ? [riskAssessmentOverride] : [] });
        }
        ...
        if (url.includes("/analytics/team-overview")) {
          return Promise.resolve({ data: teamOverviewOverride ?? { total_athletes: 0, assessed_athletes: 0, band_counts: { low: 0, moderate: 0, high: 0, critical: 0 }, athletes: [] } });
        }
        return Promise.resolve({ data: [] });
      }),
...
describe("Risk Assessment tab", () => {
  ...
  it("athlete can view their own risk assessment but has no compute button", async () => {
    riskAssessmentOverride = {
      id: "ra-1", overall_score: 72, risk_band: "high",
      biomechanical_score: 80, asymmetry_score: 60, historical_injury_score: 70,
      training_load_score: 50, fatigue_score: 30,
      breakdown: ["Test breakdown line"], data_completeness_warnings: [],
      computed_at: "2026-01-01T00:00:00", computed_by_user_id: "user-3",
    };
    renderWithProviders(<MyProfile />, { user: athleteUser });
    await waitFor(() => screen.getByText(/my profile/i));
    ...
    expect(screen.queryByText(/compute new assessment/i)).not.toBeInTheDocument();
  });

  it("coach cannot see the compute button on an athlete's risk tab (not a write role)", async () => {
    renderWithProviders(<AthleteDetail />, { route: "/athletes/profile-1", path: "/athletes/:profileId", user: coachUser });
    await waitFor(() => expect(screen.getByText("Alex Athlete")).toBeInTheDocument());
    fireEvent.click(screen.getByText("Risk Assessment"));
    await waitFor(() => expect(screen.getByText(/no risk assessment yet/i)).toBeInTheDocument());
    expect(screen.queryByText(/compute new assessment/i)).not.toBeInTheDocument();
  });

  it("physiotherapist sees the compute button and can trigger a new assessment", async () => {
    renderWithProviders(<AthleteDetail />, { route: "/athletes/profile-1", path: "/athletes/:profileId", user: physioUser });
    await waitFor(() => expect(screen.getByText("Alex Athlete")).toBeInTheDocument());
    fireEvent.click(screen.getByText("Risk Assessment"));
    await waitFor(() => expect(screen.getByText(/compute new assessment/i)).toBeInTheDocument());

    const api = (await import("../api")).default;
    fireEvent.click(screen.getByText(/compute new assessment/i));
    await waitFor(() => expect(api.post).toHaveBeenCalledWith("/athletes/profile-1/risk-assessments"));
  });

  it("shows the data-completeness warning when present", async () => {
    riskAssessmentOverride = {
      id: "ra-2", overall_score: 12, risk_band: "low",
      biomechanical_score: null, asymmetry_score: null, historical_injury_score: 12,
      training_load_score: null, fatigue_score: null,
      breakdown: ["No injury history on file -- baseline low score."],
      data_completeness_warnings: ["4 of 5 risk categories had insufficient data."],
      computed_at: "2026-01-01T00:00:00", computed_by_user_id: "user-3",
    };
    renderWithProviders(<AthleteDetail />, { route: "/athletes/profile-1", path: "/athletes/:profileId", user: physioUser });
    await waitFor(() => expect(screen.getByText("Alex Athlete")).toBeInTheDocument());
...
describe("Team Overview page", () => {
  ...
  it("shows aggregate stats and the roster table", async () => {
    teamOverviewOverride = {
      total_athletes: 2, assessed_athletes: 1,
      band_counts: { low: 0, moderate: 0, high: 1, critical: 0 },
      athletes: [
        { profile_id: "profile-1", full_name: "Risky Athlete", sport_type: "Football", latest_score: 72, latest_band: "high", computed_at: "2026-01-01T00:00:00" },
        { profile_id: "profile-2", full_name: "Unassessed Athlete", sport_type: "Rugby", latest_score: null, latest_band: null, computed_at: null },
      ],
    };
    renderWithProviders(<TeamOverview />, { user: coachUser });
    await waitFor(() => expect(screen.getByText("Risky Athlete")).toBeInTheDocument());
    expect(screen.getByText("Unassessed Athlete")).toBeInTheDocument();
    expect(screen.getByText(/currently flagged high or critical/i)).toBeInTheDocument();
  });

  it("shows an empty state when there are no athletes", async () => {
    renderWithProviders(<TeamOverview />, { user: coachUser });
    await waitFor(() => expect(screen.getByText(/no athletes yet/i)).toBeInTheDocument());
  });
});

describe("Report download buttons", () => {
  const physioUser = { id: "user-3", full_name: "Pat Physio", email: "p@test.com", role: "physiotherapist" };

  it("shows Download PDF only once a risk assessment exists", async () => {
    renderWithProviders(<AthleteDetail />, { route: "/athletes/profile-1", path: "/athletes/:profileId", user: physioUser });
    await waitFor(() => expect(screen.getByText("Alex Athlete")).toBeInTheDocument());
    fireEvent.click(screen.getByText("Risk Assessment"));
    await waitFor(() => expect(screen.getByText(/no risk assessment yet/i)).toBeInTheDocument());
    ...
    riskAssessmentOverride = {
      id: "ra-1", overall_score: 50, risk_band: "moderate",
      biomechanical_score: 50, asymmetry_score: 50, historical_injury_score: 50,
      training_load_score: 50, fatigue_score: 50,
      breakdown: [], data_completeness_warnings: [],
      computed_at: "2026-01-01T00:00:00", computed_by_user_id: "user-3",
    };
    fireEvent.click(screen.getByText(/compute new assessment/i));
  ...
  it("shows Export Excel on the Training Load tab once sessions exist", async () => {
    renderWithProviders(<AthleteDetail />, { route: "/athletes/profile-1", path: "/athletes/:profileId", user: physioUser });
    await waitFor(() => expect(screen.getByText("Alex Athlete")).toBeInTheDocument());
    fireEvent.click(screen.getByText("Training Load"));
    await waitFor(() => expect(screen.getByText(/no training sessions logged/i)).toBeInTheDocument());
