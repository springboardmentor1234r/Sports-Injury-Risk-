...
export const VideoUpload = () => {
  ...
  const confirmDeleteVideo = async () => {
    ...
    try {
      ...
      const msg = err.response?.data?.detail || "Failed to delete video record.";
      showToast(msg, "danger");
    } finally {
      setDeletingId('');
    }
    ...
    <div className="min-h-screen bg-hud-black text-white p-6 md:p-8 font-sans space-y-6">
      ...
      {videoToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-fade-in">
          <div className="hud-glass-panel p-6 max-w-sm w-full space-y-4 rounded-xl border border-hud-danger/30 shadow-2xl">
            <div className="flex items-center gap-3 text-hud-danger">
              <AlertOctagon className="w-6 h-6 shrink-0" />
              <h3 className="text-sm font-bold uppercase tracking-wider text-hud-primary">
                Delete this video?
              </h3>
            </div>
            <p className="text-xs text-hud-secondary leading-relaxed">
              This action cannot be undone. The video session <span className="text-hud-primary font-bold">"{videoToDelete.original_filename}"</span> will be permanently removed.
            </p>
            <div className="flex gap-3 justify-end pt-2">
              ...
              <Button
                ...
                loading={!!deletingId}
                className="text-xs bg-hud-danger hover:bg-hud-danger/80 text-white"
              >
                Delete
              </Button>
            </div>
