...
const AthleteDashboard = ({ user }) => {
  ...
  return (
    <div className="page-container">
      ...
      {/* Recent Analyses & Recommendations Row */}
      <div className="grid-2" style={{ marginBottom: '1.5rem' }}>
        <div className="card">
          <div style={styles.sectionHeader}>
            <h3 className="card-title">Recent Video Analyses</h3>
          </div>
          <AnalysisHistoryTable analyses={analyses.slice(0, 4)} />
        </div>

        <div className="card">
          <div style={styles.sectionHeader}>
            <h3 className="card-title">Latest AI Recommendations</h3>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
            {recommendations.length === 0 ? (
              <p style={{ color: '#6B7280', fontSize: '0.85rem' }}>No pending recommendations. Upload a video for AI feedback.</p>
            ) : (
