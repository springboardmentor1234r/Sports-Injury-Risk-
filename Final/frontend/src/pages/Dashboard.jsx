...
import { 
  ...
  Brain, Bell, History, ChevronRight, Zap, Clock, UserCheck
} from 'lucide-react';
...
const Dashboard = () => {
  ...
  // Analysis sessions state (for M3/M4 integration)
  const [sessions, setSessions] = useState([]);
  const [sessionsLoading, setSessionsLoading] = useState(false);

  // Active dashboard tab for non-athlete roles
  const [activeTab, setActiveTab] = useState('registry'); // 'registry' | 'sessions'
  ...
  const [userProfileForm, setUserProfileForm] = useState({ name: '', email: '', password: '' });
  const [userProfileUpdating, setUserProfileUpdating] = useState(false);
  ...
  const fetchCoaches = async () => {
    try {
      const res = await api.get('/athletes/coaches');
      if (Array.isArray(res.data)) {
        setAvailableCoaches(res.data);
      }
    } catch (err) {
      console.error("Failed to load registered coaches:", err);
    }
  };
  ...
  // Sync tab from URL query params (e.g. /dashboard?tab=sessions)
  useEffect(() => {
    const params = new URLSearchParams(location.search);
    const tabParam = params.get('tab');
    if (tabParam === 'sessions') {
      setActiveTab('sessions');
    } else if (tabParam === 'registry' || tabParam === 'profile') {
      setActiveTab(user?.role === 'Athlete' ? 'profile' : 'registry');
    }
  }, [location.search, user?.role]);

  // Sidebar dynamic active indicator helper
  const getSidebarItemClass = (isActive) =>
    isActive
      ? "w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-semibold bg-hud-blue/15 border border-hud-blue/30 text-hud-blue transition-all cursor-pointer text-left"
      : "w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-semibold text-gray-400 hover:text-white hover:bg-hud-dark/50 border border-transparent transition-all cursor-pointer text-left";

  const getSidebarIconClass = (isActive) =>
    isActive ? "w-4 h-4 text-hud-blue" : "w-4 h-4 text-gray-500";
  ...
  // Fetch real analysis sessions from Milestone 2
  const fetchSessions = async () => {
    setSessionsLoading(true);
    try {
      const res = await api.get('/milestone2/analysis/sessions');
      const data = res.data;
  ...
  // Open user account profile edit modal (for all roles)
  ...
  // Save user account profile updates
  const saveUserProfile = async (e) => {
    ...
    try {
      ...
      if (userProfileForm.password && userProfileForm.password.trim().length >= 6) {
        payload.password = userProfileForm.password.trim();
      }
      ...
      // If user is athlete and has a biometric profile, refresh it
      if (user?.role === 'Athlete') {
        fetchMyProfile();
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-hud-black text-hud-primary flex flex-col font-sans">
      ...
      <header className="hud-glass-panel rounded-none border-b border-hud-border px-6 md:px-10 py-4 flex justify-between items-center sticky top-0 z-40 backdrop-blur-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-md font-extrabold tracking-wider text-hud-primary uppercase block">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden md:flex items-center gap-3 font-sans text-xs text-hud-secondary">
          ...
          {/* Edit Profile Button (All authenticated roles) */}
          <Button
          ...
          >
            <UserCheck className="w-3.5 h-3.5 text-hud-blue" />
      ...
      <div className="flex-1 flex flex-col lg:flex-row">
        ...
        <aside className="lg:w-60 bg-hud-dark/30 border-r border-hud-border p-6 flex flex-col justify-between relative z-20">
          <div className="space-y-6">
            <div>
              <span className="text-[10px] font-bold tracking-wider text-gray-500 uppercase block mb-3">Data & Tracking</span>
                ...
                {user?.role === 'Athlete' ? (
                  <>
                    <button
                      ...
                      className={getSidebarItemClass((location.pathname === '/dashboard' || location.pathname === '/profile') && activeTab !== 'sessions')}
                    >
                      <User className={getSidebarIconClass((location.pathname === '/dashboard' || location.pathname === '/profile') && activeTab !== 'sessions')} />
                      <span>My Profile</span>
                    </button>
                    <button
                      onClick={() => navigate('/milestone2/upload')}
                      className={getSidebarItemClass(location.pathname === '/milestone2/upload' || location.pathname.startsWith('/video-upload'))}
                    >
                      <Film className={getSidebarIconClass(location.pathname === '/milestone2/upload' || location.pathname.startsWith('/video-upload'))} />
                      <span>Video Upload</span>
                    </button>
                    <button
                      onClick={() => navigate('/milestone2/pose')}
                      className={getSidebarItemClass(location.pathname === '/milestone2/pose' || location.pathname.startsWith('/pose-ingestion'))}
                    >
                      <Activity className={getSidebarIconClass(location.pathname === '/milestone2/pose' || location.pathname.startsWith('/pose-ingestion'))} />
                      <span>Pose Ingestion</span>
                    </button>
                    <button
                      onClick={() => navigate('/milestone2/skeleton')}
                      className={getSidebarItemClass(location.pathname === '/milestone2/skeleton' || location.pathname.startsWith('/skeleton-tracking'))}
                    >
                      <Activity className={getSidebarIconClass(location.pathname === '/milestone2/skeleton' || location.pathname.startsWith('/skeleton-tracking'))} />
                      <span>Skeleton Tracking</span>
                    </button>
                    <button
                      onClick={() => navigate('/milestone2/biomechanics')}
                      className={getSidebarItemClass(location.pathname === '/milestone2/biomechanics' || location.pathname.startsWith('/biomechanics'))}
                    >
                      <Activity className={getSidebarIconClass(location.pathname === '/milestone2/biomechanics' || location.pathname.startsWith('/biomechanics'))} />
                  ...
                  <>
                    <button
                      ...
                      className={getSidebarItemClass((location.pathname === '/dashboard' || location.pathname === '/athletes') && activeTab !== 'sessions')}
                    >
                      <Users className={getSidebarIconClass((location.pathname === '/dashboard' || location.pathname === '/athletes') && activeTab !== 'sessions')} />
            ...
            {/* Milestone 3 / 4 Navigation */}
            <div>
              <span className="text-[10px] font-bold tracking-wider text-gray-500 uppercase block mb-3">Insights & Reports</span>
              <nav className="space-y-1">
                <button
                  onClick={() => {
                    const firstCompleted = sessions.find(s => s.processing_status === 'completed');
                    if (firstCompleted) navigate(`/milestone3/intelligence/${firstCompleted.session_id}`);
                    else {
                      setActiveTab('sessions');
                      navigate('/dashboard?tab=sessions');
                    }
                  }}
                  className={getSidebarItemClass(location.pathname.startsWith('/milestone3') || location.pathname.startsWith('/athlete-intelligence'))}
                >
                  <Brain className={getSidebarIconClass(location.pathname.startsWith('/milestone3') || location.pathname.startsWith('/athlete-intelligence'))} />
                  <span>Athlete Intelligence</span>
                </button>
                <button
                  onClick={() => {
                    const firstCompleted = sessions.find(s => s.processing_status === 'completed');
                    if (firstCompleted) navigate(`/milestone4/report/${firstCompleted.session_id}`);
                    else {
                      setActiveTab('sessions');
                      navigate('/dashboard?tab=sessions');
                    }
                  }}
                  className={getSidebarItemClass(location.pathname.startsWith('/milestone4/report') || location.pathname.startsWith('/evaluation-report'))}
                >
                  <FileText className={getSidebarIconClass(location.pathname.startsWith('/milestone4/report') || location.pathname.startsWith('/evaluation-report'))} />
                ...
                <button
                  ...
                  className={getSidebarItemClass((location.pathname === '/dashboard' && activeTab === 'sessions') || location.pathname.startsWith('/milestone4/history') || location.pathname.startsWith('/milestone4/notifications'))}
                >
                  <History className={getSidebarIconClass((location.pathname === '/dashboard' && activeTab === 'sessions') || location.pathname.startsWith('/milestone4/history') || location.pathname.startsWith('/milestone4/notifications'))} />
        ...
        {/* CONTENT PANEL */}
        <main className="flex-1 p-6 md:p-8 relative z-10 overflow-y-auto w-full max-w-full">
          ...
          {user?.role === 'Athlete' && (
            ...
            >
              <div className="border-b border-hud-border pb-4 flex justify-between items-end">
                <div>
                  <h1 className="text-2xl font-bold tracking-tight text-hud-primary">
                    Biometric Profile
                  </h1>
                  <p className="text-hud-secondary text-xs mt-1">
                ...
                <Card className="space-y-6 text-sm">
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-6 p-6 rounded-xl bg-hud-dark/20 border border-hud-border">
                    ...
                    <div className="flex-1 space-y-2 text-center sm:text-left">
                      <h3 className="text-xl font-bold text-hud-primary uppercase">{myProfile.full_name}</h3>
                  ...
                  {/* Core Metrics Grid */}
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-5 rounded-xl bg-hud-dark/20 border border-hud-border">
                    <div className="space-y-1">
                      <span className="text-xs text-hud-secondary font-medium block">Age / Gender</span>
                      <span className="text-hud-primary font-bold text-sm block">{myProfile.age} yrs / {myProfile.gender}</span>
                    </div>
                    <div className="space-y-1">
                      <span className="text-xs text-hud-secondary font-medium block">Height / Weight</span>
                      <span className="text-hud-primary font-bold text-sm block">{myProfile.height} / {myProfile.weight}</span>
                    </div>
                    <div className="space-y-1">
                      <span className="text-xs text-hud-secondary font-medium block">Position</span>
                      <span className="text-hud-primary font-bold text-sm block">{myProfile.playing_position || 'N/A'}</span>
                    </div>
                    <div className="space-y-1">
                      <span className="text-xs text-hud-secondary font-medium block">Training Load</span>
                      <span className="text-hud-green font-bold text-sm block">{myProfile.training_load}</span>
                    </div>
                  </div>

                  {/* Detailed Biometric, Health & History Grid */}
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                    
                    {/* Biographical Metrics */}
                    <div className="min-h-[145px] space-y-2 p-5 rounded-xl bg-hud-dark/15 border border-hud-border flex flex-col justify-between">
                      <div>
                        <span className="text-xs text-hud-blue uppercase font-bold tracking-wider block border-b border-hud-border pb-1.5 flex items-center gap-1.5 mb-2.5">
                          <Info className="w-4 h-4" />
                          Biographical Metrics
                        </span>
                        <div className="space-y-2 text-xs">
                          <div className="flex justify-between items-center">
                            <span className="text-hud-secondary font-medium">Experience:</span>
                            <span className="text-hud-primary font-semibold">{myProfile.experience || 'N/A'}</span>
                          </div>
                          <div className="flex justify-between items-center">
                            <span className="text-hud-secondary font-medium">Coach:</span>
                            <span className="text-hud-primary font-semibold">{myProfile.coach_name || 'N/A'}</span>
                          </div>
                          <div className="flex justify-between items-center">
                            <span className="text-hud-secondary font-medium">Emergency Contact:</span>
                            <span className="text-hud-primary font-semibold">{myProfile.emergency_contact || 'N/A'}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="min-h-[145px] space-y-2 p-5 rounded-xl bg-hud-dark/15 border border-hud-border flex flex-col justify-between">
                      <div>
                        ...
                        <div className="space-y-2 text-xs">
                          <div className="flex justify-between items-center">
                            <span className="text-hud-secondary font-medium">Status flag:</span>
                            <span className="text-hud-primary font-bold">{myProfile.disability_status || 'No'}</span>
                          </div>
                          {myProfile.disability_status === 'Yes' && (
                            <>
                              <div className="flex justify-between items-center">
                                <span className="text-hud-secondary font-medium">Type:</span>
                                <span className="text-hud-primary font-semibold">{myProfile.disability_type || 'N/A'}</span>
                              </div>
                              <div className="flex justify-between items-center">
                                <span className="text-hud-secondary font-medium">Device:</span>
                                <span className="text-hud-primary font-semibold">{myProfile.assistive_device || 'N/A'}</span>
                              </div>
                            </>
                          )}
                        </div>
                    ...
                    <div className="min-h-[145px] space-y-2 p-5 rounded-xl bg-hud-dark/15 border border-hud-border flex flex-col justify-between">
                      <div>
                        <span className="text-xs text-hud-danger uppercase font-bold tracking-wider block border-b border-hud-danger/20 pb-1.5 flex items-center gap-1.5 mb-2.5">
                          <AlertOctagon className="w-4 h-4" />
                          Injury History
                        </span>
                        <p className="text-hud-primary text-xs leading-relaxed whitespace-pre-line">{myProfile.injury_history || 'No recorded prior injuries.'}</p>
                      </div>
                    </div>
                    ...
                    <div className="min-h-[145px] space-y-2 p-5 rounded-xl bg-hud-dark/15 border border-hud-border flex flex-col justify-between">
                      <div>
                        <span className="text-xs text-hud-warning uppercase font-bold tracking-wider block border-b border-hud-warning/20 pb-1.5 flex items-center gap-1.5 mb-2.5">
                          <FileText className="w-4 h-4" />
                          Medical Diagnostics
                        </span>
                        <p className="text-hud-primary text-xs leading-relaxed whitespace-pre-line">{myProfile.medical_notes || 'Standard physiological state.'}</p>
                      </div>
                    </div>
                  ...
                  <div className="flex justify-between text-[10px] text-hud-muted border-t border-hud-border pt-4">
          ...
          {user?.role !== 'Athlete' && activeTab === 'registry' && (
            <motion.div 
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className="space-y-6"
          ...
          {/* ── ANALYSIS SESSIONS PANEL (all roles) ─────────────────────────── */}
          {(activeTab === 'sessions' || user?.role === 'Athlete') && (
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className="space-y-6 w-full max-w-full"
            >
              <div className="border-b border-hud-border pb-4 flex justify-between items-end">
                <div>
                  <h1 className="text-2xl font-bold tracking-tight">Analysis Sessions</h1>
                  <p className="text-gray-400 text-xs mt-1">
                    Real Milestone 2 sessions. Click <strong>View Intelligence</strong> to open Milestone 3.
                  </p>
                </div>
                <div className="flex gap-2">
                  <Button onClick={fetchSessions} variant="outline" className="text-xs">
                    <RefreshCw className="w-3.5 h-3.5" />
                    <span>Refresh</span>
                  </Button>
                  <Button onClick={() => navigate('/milestone2/upload')} className="text-xs">
              ...
              {sessionsLoading ? (
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                  {sessions.map((session) => (
                    <Card key={session.session_id} className="p-5 hover:border-hud-blue/40 transition-colors flex flex-col justify-between space-y-4">
                          ...
                          <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider ${
                            session.processing_status === 'completed'
                              ...
                              : session.processing_status === 'processing'
                                ? 'bg-hud-warning/10 text-hud-warning border border-hud-warning/30'
                                : 'bg-hud-dark/50 text-gray-500 border border-hud-border'
                          }`}>
                            {session.processing_status}
                          </span>
                          {session.has_intelligence && (
                            <span className="px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider bg-hud-blue/15 text-hud-blue border border-hud-blue/30">
                              <Zap className="w-2.5 h-2.5 inline mr-0.5" />AI Ready
                            </span>
                          )}
                        </div>
                        <h3 className="text-sm font-bold text-white truncate">{session.session_name}</h3>
                        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-400">
                          <span>Athlete: <span className="text-gray-200 font-medium">{session.athlete_name}</span></span>
                          <span>Activity: <span className="text-gray-200 font-medium">{session.activity_type || 'N/A'}</span></span>
                          {session.created_at && (
                      ...
                      <div className="flex gap-2 flex-wrap pt-2 border-t border-hud-border/40">
                        {session.processing_status === 'completed' ? (
                          <>
                            <button
                              onClick={() => navigate(`/milestone3/intelligence/${session.session_id}`)}
                              className="flex items-center gap-1.5 px-3 py-1.5 bg-hud-blue hover:bg-hud-blue/85 text-white rounded-lg text-xs font-bold cursor-pointer transition-colors"
                            >
                            ...
                            <button
                              onClick={() => navigate(`/milestone4/report/${session.session_id}`)}
                              className="flex items-center gap-1.5 px-3 py-1.5 border border-hud-border hover:border-hud-blue hover:text-hud-blue text-gray-400 rounded-lg text-xs font-bold cursor-pointer transition-colors"
                            >
                            ...
                            {session.athlete_id && (
                              <button
                                onClick={() => navigate(`/milestone4/history/${session.athlete_id}`)}
                                className="flex items-center gap-1.5 px-3 py-1.5 border border-hud-border hover:border-hud-blue hover:text-hud-blue text-gray-400 rounded-lg text-xs font-bold cursor-pointer transition-colors"
                              >
                          ...
                          <span className="px-3 py-1.5 text-xs text-gray-500 font-mono border border-hud-border/30 rounded-lg">
                            {session.processing_status === 'processing' ? 'Processing...' : 'Not Ready'}
                          </span>
                        )}
                      </div>
                    </Card>
                ...
                <Card className="p-8 text-center">
                  <Activity className="w-10 h-10 text-gray-500 mx-auto mb-4" />
                  <h3 className="text-sm font-bold text-white uppercase mb-2">No Analysis Sessions Yet</h3>
                  <p className="text-gray-400 text-xs leading-relaxed mb-6">
                    Upload a video to start your first analysis session.
                  </p>
                  <Button onClick={() => navigate('/milestone2/upload')} className="mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
            ...
            <div className="sm:col-span-1">
              ...
              {availableCoaches.length > 0 ? (
                <select
                  className="w-full px-3 py-2.5 bg-hud-dark border border-hud-border rounded-lg text-xs text-hud-primary cursor-pointer"
                  value={athleteForm.coach_id || athleteForm.coach_name}
                  onChange={(e) => {
                    const selectedCoach = availableCoaches.find(c => c.id === e.target.value || c.name === e.target.value);
                    setAthleteForm({
                      ...athleteForm,
                      coach_name: selectedCoach ? selectedCoach.name : e.target.value,
      ...
      {/* MODAL 4: USER ACCOUNT PROFILE EDIT (ALL AUTHENTICATED ROLES) */}
      <Modal
        ...
        maxWidth="max-w-md"
      >
        <form onSubmit={saveUserProfile} className="space-y-4 text-xs">
          
          <div className="p-3.5 rounded-xl bg-hud-dark/30 border border-hud-border flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <ShieldCheck className="w-4 h-4 text-hud-green" />
              <div>
                <span className="text-[10px] text-hud-muted block uppercase font-bold">Assigned Role</span>
                <span className="text-xs font-bold text-hud-primary uppercase">{user?.role}</span>
              </div>
            </div>
            <span className="px-2.5 py-0.5 rounded-full bg-hud-blue/15 border border-hud-blue/30 text-hud-blue text-[10px] font-semibold uppercase">
              Authorized Account
            </span>
          </div>

          <div className="space-y-1.5">
            <label className="text-xs text-hud-secondary font-medium block">Full Name *</label>
            <Input
              type="text"
          ...
          <div className="space-y-1.5">
            <label className="text-xs text-hud-secondary font-medium block">Email Address *</label>
            <Input
              type="email"
              placeholder="e.g. coach@kineticguard.com"
          ...
          <div className="space-y-1.5">
            <label className="text-xs text-hud-secondary font-medium block">
              New Password <span className="text-hud-muted font-normal">(optional, min 6 chars)</span>
            </label>
            <Input
              type="password"
              placeholder="Leave blank to keep current password"
              value={userProfileForm.password}
              onChange={(e) => setUserProfileForm(prev => ({ ...prev, password: e.target.value }))}
            />
          </div>
          ...
          <div className="flex gap-3 justify-end pt-4 border-t border-hud-border">
            <Button
              ...
              variant="secondary"
              disabled={userProfileUpdating}
            >
              Cancel
            </Button>
            <Button
              type="submit"
              loading={userProfileUpdating}
            >
              Save Profile
            </Button>
          </div>
