...
### ✅ Milestone 3 — Athlete Intelligence & Biomechanical Risk Scoring

Milestone 3 converts kinematic telemetry into actionable clinical intelligence:
* **Kinematic Anomaly Detection**: Identifies dynamic joint stress patterns (knee valgus collapse, trunk lateral sway, hip flexion).
* **Bilateral Limb Asymmetry Matrix**: Compares left vs right joint angle deviations to uncover unilateral compensations.
* **Acute-to-Chronic Workload Ratio (ACWR)**: Evaluates short-term strain against chronic fitness conditioning.
* **Composite Injury Risk Engine**: Generates a weighted 0–100% risk index with Low, Moderate, and High classifications.
* **Interactive UI**: Radar charts, risk gauge, dynamic joint vulnerability cards, and pipeline execution monitor.

---

### ✅ Milestone 4 — Clinical Evaluation Reports & Longitudinal Logs

Milestone 4 delivers executive reports and historical tracking:
* **Clinical Evaluation Report**: Diagnostic summary with risk category badge, key kinematic observations, and personalized corrective drills.
* **Date-Range Filtered Analysis Logs**: Synchronized session directory querying historical records between selected Start and End dates.
* **Proactive Anomaly Alerts**: Real-time notification dispatching for high-risk movement sessions (>70% risk or >20% asymmetry).

---

## 📂 Repository Milestone Structure

```
Sports-Injury-Risk-
│
├── Milestone_1/
│   ├── backend/                     # M1 FastAPI backend (auth, athletes, models, schemas, tests)
│   ├── frontend/                    # M1 React UI (Dashboard, Login, Register, components)
│   ├── shared/                      # Prototype templates & shared database files
│   ├── requirements.txt             # M1 Python dependencies
│   ├── summary.md                   # M1 Milestone summary
│   └── README.md                    # M1 Documentation
│
├── Milestone_2/
│   ├── backend/                     # M2 Video, Pose & Biomechanics backend services
│   ├── frontend/                    # M2 Pose, Skeleton & Biomechanics UI components
│   ├── models/                      # Pose & Skeleton data schemas and models
│   ├── src/                         # Core computer vision & joint kinematic algorithms
│   ├── outputs/                     # Sample tracking & telemetry outputs
│   ├── requirements.txt             # M2 dependencies (OpenCV, MediaPipe, etc.)
│   └── README.md                    # M2 Documentation
│
├── Milestone_3/
│   ├── backend/                     # M3 FastAPI backend (anomalies, risk scores, intelligence)
│   ├── frontend/                    # M3 React components & Athlete Intelligence dashboard
│   ├── models/                      # M3 Anomaly & Risk score models
│   ├── services/                    # M3 Biomechanical calculation & ACWR fatigue engine
│   └── README.md                    # M3 Documentation
│
├── Milestone_4/
│   ├── backend/                     # M4 FastAPI backend (reports, history, notifications)
│   ├── frontend/                    # M4 Evaluation Reports, History & Alerts UI
│   ├── tests/                       # M4 backend and frontend test suites
│   ├── scripts/                     # Verification and pipeline scripts
│   └── README.md                    # M4 Documentation
│
├── .gitignore                       # Clean ignores (node_modules, venv, __pycache__, dist)
...
### 1. Integrated Backend Setup
```bash
# From repository root
.\backend\venv\Scripts\python.exe -m uvicorn Milestone_4.backend.main:app --host 127.0.0.1 --port 8000
```
Interactive API docs: `http://127.0.0.1:8000/docs`

### 2. Integrated Frontend Setup
```bash
cd frontend
npm install
npm run dev
...
|---|---|:---:|
| `/login` | User Authentication | Milestone 1 |
| `/register` | Athlete / Coach Registration | Milestone 1 |
| `/dashboard` | Biometric Profile & Athlete Registry | Milestone 1 |
| `/milestone2/upload` | Video Upload & Processing | Milestone 2 |
| `/milestone2/pose` | 33-Keypoint Pose Estimation | Milestone 2 |
| `/milestone2/skeleton` | 2D/3D Skeleton Tracking | Milestone 2 |
| `/milestone2/biomechanics` | Joint Biomechanics Telemetry | Milestone 2 |
| `/milestone3/intelligence/:sessionId` | Athlete Intelligence & Risk Scoring | Milestone 3 |
| `/milestone4/report/:sessionId` | Clinical Evaluation Report | Milestone 4 |
| `/milestone4/history/:athleteId` | Date-Filtered Historical Logs | Milestone 4 |
| `/milestone4/notifications/:athleteId` | Proactive Risk Alerts & Notifications | Milestone 4 |
