...
async def upload_video(
    ...
    if role == "Athlete":
        ...
        if not ath_doc or (ath_doc.get("full_name", "").lower() != user_name.lower() and (ath_doc.get("created_by") or "").lower() != current_user.get("email", "").lower()):
            raise HTTPException(
...
async def list_videos(
    ...
    Coaches can only view videos of athletes assigned to them.
    """
    role = current_user.get("role")
    user_name = current_user.get("name")
    ...
    if role == "Athlete":
        ...
        ath_ids = [str(a["_id"]) for a in coach_athletes]
        for a in coach_athletes:
            if a.get("athlete_id"):
                ath_ids.append(a["athlete_id"])
        
        if athlete_id:
            if athlete_id not in ath_ids:
                return {"videos": [], "total": 0}
            query["athlete_id"] = athlete_id
        else:
            query["$or"] = [
                {"athlete_id": {"$in": ath_ids}},
                {"uploaded_by": coach_email}
            ]
    elif athlete_id:
        query["athlete_id"] = athlete_id
...
async def get_video_details(
    ...
    # Enforce read permissions check
    if role == "Athlete":
        ...
        try:
            ...
            pass
        if not ath_doc:
            ath_doc = await db["athletes"].find_one({"athlete_id": video_doc["athlete_id"]})
        
        is_owner = (video_doc.get("uploaded_by") or "").strip().lower() == coach_email
        if ath_doc:
            ath_coach_id = str(ath_doc.get("coach_id") or "").strip().lower()
            ath_coach_name = (ath_doc.get("coach_name") or "").strip().lower()
            ath_created_by = (ath_doc.get("created_by") or "").strip().lower()
            if (
        ...
        if not is_owner:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Access denied. Video belongs to another coach's athlete."
            )
