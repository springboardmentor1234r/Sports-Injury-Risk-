...
export function BodyHeatmapGraphic({ heatmapData }) {
...
export function JointAngleRadarChart({ metrics }) {
  // Normalize 6 joint dimensions to percentages
  ...
  // Radar points on hexagonal polygon
  const center = 150;
  const radius = 100;

  const points = [
    { label: 'Knee Valgus', val: valgus, angle: 0 },
    { label: 'Asymmetry', val: asymmetry, angle: 60 },
    { label: 'Stiff Landing', val: landing, angle: 120 },
    { label: 'Trunk Lean', val: trunk, angle: 180 },
    { label: 'COM Sway', val: drift, angle: 240 },
    { label: 'Training Load', val: load, angle: 300 }
  ];
...
export function RiskTrendAreaChart({ history }) {
