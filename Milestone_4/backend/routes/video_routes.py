...
from services.notifications import generate_notifications_for_risk_assessment
...
def process_video_task(video_id: int):
    ...
    try:
        ...
        try:
            ...
            # also wrapped so a notification hiccup never affects the
            # video/biomechanics/risk results already saved above.
            try:
                if athlete_obj is not None:
                    generate_notifications_for_risk_assessment(db, athlete_obj.user_id, video, risk_data)
            except Exception as notify_err:  # noqa: BLE001
                print(f"Notification generation failed for video {video.id}: {notify_err}")
