    ...
    <header className="sticky top-0 z-40 w-full glass-panel border-b border-slate-800 px-4 lg:px-8 py-3.5">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3 sm:space-x-4">
          ...
          {user && (
            <div className="flex items-center space-x-2 bg-slate-900/80 border border-slate-800 rounded-lg px-2.5 py-1.5 text-xs">
              <Sparkles className="w-3.5 h-3.5 text-cyan-400 hidden md:block" />
              <span className="text-slate-400 hidden md:inline font-medium">View Mode:</span>
              <select
                value={user.role}
                onChange={(e) => switchRole(e.target.value)}
                className="bg-slate-800 text-cyan-300 font-semibold rounded px-2 py-1 outline-none border border-cyan-900/60 focus:border-cyan-500 cursor-pointer"
              >
                {roles.map((r) => (
