                ...
                {filteredAthletes.length === 0 ? (
                  <tr>
                    <td colSpan={7} style={{ textAlign: 'center', padding: '2.5rem', color: '#6B7280' }}>
                      ...
                      <p style={{ fontWeight: '600' }}>No registered athletes found.</p>
                      <span style={{ fontSize: '0.8rem' }}>Newly registered athlete accounts will automatically appear here.</span>
                    </td>
                  </tr>
                ) : (
                  filteredAthletes.map((ath) => (
                    <tr key={ath.id}>
                      <td>
                        <div>
                          <strong style={{ color: '#1F2937', fontSize: '0.92rem' }}>{ath.name}</strong>
                      ...
                      <td style={{ fontSize: '0.85rem', color: '#4B5563' }}>
                        {ath.email || `${ath.name.toLowerCase().replace(/\s+/g, '')}@athletiq.ai`}
                      </td>
                      <td>
                        <span style={{ fontWeight: '600', color: '#1F2937' }}>{ath.sport_type}</span>
                      ...
                      <td>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.2rem' }}>
                          ...
                          <span style={{ fontSize: '0.82rem', fontWeight: '800', color: '#1F2937' }}>
                            Score: {ath.recent_risk_score} / 100
                          </span>
                        </div>
                      </td>
                      <td>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.15rem' }}>
                          <span style={{ fontWeight: '700', color: '#15803D', fontSize: '0.85rem' }}>
                            Movement: {ath.movement_quality_score}%
                          </span>
                          <span style={{ fontSize: '0.75rem', color: '#6B7280' }}>
                            {ath.sessions_analyzed > 0 ? `${ath.sessions_analyzed} Videos Analyzed` : 'No Videos Analyzed'}
                          </span>
                        </div>
                      </td>
                      <td style={{ fontSize: '0.8rem', color: '#6B7280' }}>
                        {ath.created_at ? new Date(ath.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 'Recently'}
                      </td>
                      <td>
