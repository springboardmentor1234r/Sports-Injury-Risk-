...
# Must be set before importing app.config/database so verification never touches
# the developer database or a configured PostgreSQL instance.
...
def run_tests():
    ...
    res = client.get("/api/athletes/profile")
    ...
    # Background processing runs within TestClient's request lifecycle. Validate
    # the persisted explainable M3 assessment and its specified weights.
    res = client.get(f"/api/intelligence/athletes/{athlete_id}", headers=headers)
    assert res.status_code == 200
    intelligence = res.json()
    assert intelligence["analytical_disclaimer"].startswith("Analytical risk estimate")
    weights = intelligence["scoring_breakdown"]["weights"]
    assert weights == {
    ...
    assert len(intelligence["risk_predictions"]) == 6
    print("[+] Persistent anomaly detection, transparent weighted scoring, risk categories, and recommendations validated.")

    res = client.post(f"/api/intelligence/athletes/{athlete_id}/assess", headers=headers)
    assert res.status_code == 200
    print("[+] Manual assessment snapshot endpoint validated.")
    ...
    assert res.status_code == 200
    db = TestingSessionLocal()
    reset_token = db.query(models.User).filter(models.User.email == athlete_data["email"]).first().reset_token
    db.close()
    ...
    res = client.get("/api/intelligence/executive", headers=coach_headers)
    assert res.status_code == 200
    assert "risk_distribution" in res.json()
    print("[+] Role-protected executive intelligence dashboard validated.")
