...
const ROLE_ACTIONS: Record<AppRole, ActionCard[]> = {
  athlete: [
    ...
    },
    {
      ...
      description: "See movement quality scores from your videos.",
      to: "/analysis",
    },
  ],
  coach: [
    ...
    },
    {
      icon: FileBarChart,
      title: "Reports",
      description: "Injury-risk reports (coming in Milestone 3).",
      to: "/reports",
    },
  ],
  physiotherapist: [
  ...
  sports_scientist: [
    ...
    },
    {
      ...
      description: "Run pose estimation and biomechanical analysis on submitted clips.",
      to: "/video-analysis",
    },
    {
      icon: FileBarChart,
...
function Dashboard() {
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      ...
      const analysesQuery = supabase
        .from("pose_analyses")
        .select("movement_quality_score, athlete_user_id");
      const { data: analysesRows } = isAthlete
        ? await analysesQuery.eq("athlete_user_id", user!.id)
        : await analysesQuery;

      const scored = analysesRows ?? [];
      const avgQuality = scored.length
        ? Math.round(
            scored.reduce((sum, a) => sum + (a.movement_quality_score ?? 0), 0) / scored.length,
          )
        : null;
  ...
  const cards = [
    ...
    {
      icon: ShieldCheck,
      label: isAthlete ? "Your avg. movement quality" : "Avg. movement quality (all athletes)",
