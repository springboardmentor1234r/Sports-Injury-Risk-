...
export default function InjuryRiskDashboard() {
  ...
  const handleDownloadAthleteReport = async (format) => {
    ...
    try {
      ...
      await downloadFile(
        `${API_BASE}/reports/athlete/me/${format}`,
        headers,
        `athlete_performance_report.${ext}`
      );
    } catch {
  ...
  const handleDownloadVideoReport = async (videoId) => {
    setDownloading(`video-${videoId}`);
    try {
      await downloadFile(
        `${API_BASE}/reports/videos/${videoId}/pdf`,
        headers,
        `video_${videoId}_report.pdf`
      );
    } catch {
      setError("Couldn't generate the report — try again.");
    } finally {
                    ...
                    {isOpen && (
                      <div style={styles.expandBox}>
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            handleDownloadVideoReport(h.video_id);
                          }}
                          disabled={downloading === `video-${h.video_id}`}
                        >
                          {downloading === `video-${h.video_id}` ? "Generating…" : "📄 Download this video's PDF"}
                        </button>
                      </div>
                    )}
                  </div>
...
const styles = {
  ...
  downloadBtn: {
    background: "#ede9fe", color: "#6d28d9", border: "none", padding: "12px 18px",
    borderRadius: "16px", cursor: "pointer", fontWeight: "600", fontSize: "14px",
  },
  smallDownloadBtn: {
    background: "#ede9fe", color: "#6d28d9", border: "none", padding: "8px 14px",
    borderRadius: "12px", cursor: "pointer", fontWeight: "600", fontSize: "12px", marginTop: 8,
  },
