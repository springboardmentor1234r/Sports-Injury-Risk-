...
let visionFilesetPromise: ReturnType<typeof import("@mediapipe/tasks-vision").FilesetResolver.forVisionTasks> | null = null;

/**
 * The MediaPipe "vision fileset" (WASM binaries + runtime) is expensive to
 * load — fetched over the network — but stateless, so it's safe to cache
 * and reuse across every analysis for the whole session.
 */
...
/**
 * Unlike the fileset above, the PoseLandmarker *instance* itself is
 * stateful in VIDEO running mode: MediaPipe requires every timestamp
 * passed to `detectForVideo` to be strictly greater than the last one that
 * instance has ever seen. Caching a single landmarker across multiple
 * analysis runs (different videos, or a re-run of the same video) breaks
 * the second and every subsequent run, because each run restarts its
 * timestamps from 0 — which is now behind where the shared instance left
 * off. So a fresh instance is created per call and closed when done; only
 * the fileset above is shared.
 */
...
export async function estimatePoseFromVideoUrl(
  ...
  const landmarker = await createLandmarker().catch((err) => {
    throw new PoseEstimationError(
