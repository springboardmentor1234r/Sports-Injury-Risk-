...
function AthletesList() {
  ...
  const isCoach = roles.includes("coach") && !roles.includes("administrator");
    ...
    <div className="mx-auto max-w-6xl px-6 py-10">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="font-display text-3xl font-bold">
            {isCoach ? "My athletes" : "Athletes"}
          </h1>
          <p className="mt-2 text-muted-foreground">
            {isCoach
              ? "Athletes assigned to you. Other coaches' athletes stay private to them."
              : "All athletes in the organisation — profiles, training load and injury history."}
          </p>
        </div>
        {canManage && (
          <Button asChild>
      ...
      <div className="mt-6 flex flex-wrap items-center gap-3">
        ...
        <Select value={load} onValueChange={setLoad}>
          <SelectTrigger className="w-[170px]">
            <SelectValue placeholder="Training load" />
          </SelectTrigger>
          <SelectContent>
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    {a.team_club && (
                      <div className="mt-0.5 text-xs text-muted-foreground">{a.team_club}</div>
                    )}
                    <div className="mt-0.5 text-xs text-muted-foreground">
                      Coach: {a.coach_name || (a.coach_user_id ? "Assigned" : "Unassigned")}
                    </div>
                  </div>
                  {a.training_load && (
                    <Badge
                      className={`${LOAD_TONE[a.training_load] ?? "bg-muted text-muted-foreground"} border-0`}
                    >
