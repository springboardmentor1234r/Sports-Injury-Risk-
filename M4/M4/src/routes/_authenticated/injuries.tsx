  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(
          "id, full_name, sport_type, position, training_load, injury_history, current_medical_conditions",
        )
        .order("full_name");
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Athletes with recorded injury history or medical conditions.
          </p>
        </div>
      </div>
              ...
              >
                ...
                {a.injury_history && (
                  <p className="mt-3 line-clamp-3 text-sm text-muted-foreground">
                    <span className="font-medium text-foreground">Injury: </span>
                    {a.injury_history}
                  </p>
                )}
