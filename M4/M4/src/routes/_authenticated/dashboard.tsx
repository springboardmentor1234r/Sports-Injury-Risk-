...
import { raiseRecoveryReminderIfStale } from "@/lib/alerts";
    ...
    queryFn: async () => {
      ...
      if (isAthlete) {
        ...
        const { data: athletes } = await supabase
          .from("athlete_profiles")
          .select("user_id, full_name, training_load, injury_history, current_medical_conditions");
        ...
        // Notification & Alert System: recovery reminders. Opportunistic
        // check (no cron in this stack) — idempotent server-side via
        // raiseRecoveryReminderIfStale's own dedup, so it's safe to run on
        // every staff dashboard load rather than only on a schedule.
        for (const athlete of athletes ?? []) {
          if (!athlete.user_id) continue;
          const history = byAthlete.get(athlete.user_id) ?? [];
          const sorted = [...history].sort(
            (a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime(),
          );
          const latest = sorted[0];
          raiseRecoveryReminderIfStale({
            athleteUserId: athlete.user_id,
            athleteName: athlete.full_name,
            lastAnalysisAt: latest?.createdAt ?? null,
            lastRiskFlagCount: latest?.riskFlags?.length ?? 0,
          }).catch((err) => console.error("[dashboard] recovery reminder failed:", err));
        }
      }
