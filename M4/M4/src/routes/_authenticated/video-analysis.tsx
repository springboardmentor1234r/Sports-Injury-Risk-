...
import { raiseRiskAlert, raiseAssessmentCompleteAlert } from "@/lib/alerts";
...
function VideoAnalysis() {
  ...
  const runAnalysis = useMutation({
    mutationFn: async (submission: Submission) => {
      ...
      // Notification & Alert System: risk alerts fire for high/critical only;
      // assessment-complete always notifies the athlete their analysis is ready
      // (subject to their own notification preferences).
      const athleteName = submission.profiles?.full_name ?? "Athlete";
      await Promise.all([
        raiseRiskAlert({
